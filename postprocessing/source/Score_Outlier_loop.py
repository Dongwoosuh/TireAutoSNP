...
__all__ = ['max_stress_after_outlier_removal', 'max_LE_after_outlier_removal']

def max_stress_after_outlier_removal():
    ...
    for file_name in csv_files:
        ...
        def remove_upper_outliers_zscore(data, threshold=8):
...
def max_LE_after_outlier_removal():
    # 분석할 모든 CSV 파일 찾기 (현재 디렉터리에서)
    directory_path = "./results/LE_all"  # 현재 디렉터리

    if not os.path.exists(directory_path):
        print(f"Directory {directory_path} does not exist. Skipping LE processing.")
        return
    ...
    for file_name in csv_files:
        ...
        # 이상치 제거 후 최대 LE 찾기
        max_le = filtered_df.iloc[:, 1:].max().max()
        # 결과 저장
        results.append([base_file_name, max_le])

        # 결과 출력
        print("Filtered Max LE:", max_le)
        print("\nNumber of Outliers Removed per Frame:")
        for frame, count in outlier_counts.items():
            print(f"{frame}: {count} values removed")
    ...
    # 결과를 DataFrame으로 변환 후 CSV 저장
    results_df = pd.DataFrame(results, columns=["File Name", "Max LE After Outlier Removal"])
    output_filename = "Max_LE_After_Outlier_Removal.csv"
    results_df.to_csv(os.path.join('results', output_filename), index=False)
    print("\n All LE CSV files have been processed successfully!")

if __name__ == '__main__':
    max_stress_after_outlier_removal()
    max_LE_after_outlier_removal()
